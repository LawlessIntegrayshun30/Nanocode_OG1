...
async def generate_nanocode(
    ...
    except IdempotencyLoopError as exc:
        logger.warning("Repeated request blocked by model idempotency guard", extra={"error": str(exc)})
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Repeated identical request blocked by idempotency guard",
        ) from exc
    ...
    # Only include safe metadata fields; exclude raw prompt to prevent leaking user input
    safe_metadata = {
        k: v for k, v in raw.get("metadata", {}).items()
        if k not in ["prompt"]  # Exclude prompt to prevent accidental exposure
    }
    raw["metadata"] = safe_metadata
